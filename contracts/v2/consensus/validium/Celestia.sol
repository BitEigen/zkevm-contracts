    ...
    OwnableUpgradeable
{
    ...
    // Size of a BlobPointer in bytes
    // BlobHeight - 8 bytes
    // TxCommitment - 32 bytes
    uint256 internal constant _BLOB_POINTER_SIZE = 40;
    ...
    /**
     * @dev Thrown when the da signer is not trusted sequencer address
     */
    ...
    function verifyMessage(
        ...
        if (
            ...
            (signaturesAndAddrs.length - splitByte) % _BLOB_POINTER_SIZE != 0
        ) {
    ...
    function setTrustedSequencer(
        address _trustedSequencerAddress
    ) public onlyOwner {
        trustedSequencer = _trustedSequencerAddress;
    }
}
