...
import { ICreateRollupParams, IDeployParams, INetwork, loadCreateRollupParams, loadDeployParams, loadGenesis } from "./utils";
...
interface IConfig {
  createRollupOutput: ICreateRollupParams;
  createRollupParams: ICreateRollupParams;
  deployOutput: IDeployParams;
  genesisOutput: INetwork;
}
...
async function updateConfigTask(taskArgs: { rpc: string, wsrpc: string }, hre: HardhatRuntimeEnvironment) {
  ...
  const genesisOutput = await loadGenesis(path.join(__dirname, "../deployment/v2/genesis.json"));
  ...
  setKeystore();
  updateGenesisJson(hre, chainId, config);
  updateNodeConfigToml(taskArgs.rpc, chainId, nodeConfigTomlPath, config);
  updateDACConfigToml(taskArgs.rpc, taskArgs.wsrpc, dacConfigTomlPath, config);
  updateBridgeConfigToml(taskArgs.rpc, bridgeConfigTomlPath, config);
}
...
function updateGenesisJson(hre: HardhatRuntimeEnvironment, chainId: number, config: IConfig) {
  const { createRollupOutput, deployOutput, genesisOutput } = config;
...
function updateNodeConfigToml(l1RpcUrl: string, chainId: number, configTomlPath: string, config: IConfig) {
  const { createRollupParams, deployOutput } = config;
...
function updateDACConfigToml(l1RpcUrl: string, l1WsRpcUrl: string, configTomlPath: string, config: IConfig) {
  const { createRollupOutput, deployOutput } = config;
  let configContent = fs.readFileSync(configTomlPath, "utf8");

  configContent = configContent.replace(
    /(PolygonValidiumAddress\s*=\s*)".*"/,
    `$1"${createRollupOutput.rollupAddress}"`
  );
  configContent = configContent.replace(
    /(DataCommitteeAddress\s*=\s*)".*"/,
    `$1"${createRollupOutput.polygonDataCommitteeAddress}"`
  );
  configContent = configContent.replace(/(RpcURL\s*=\s*)".*"/, `$1"${l1RpcUrl}"`);
  configContent = configContent.replace(/(WsURL\s*=\s*)".*"/, `$1"${l1WsRpcUrl}"`);
  configContent = configContent.replace(
    /(GenesisBlock\s*=\s*)".*"/,
    `$1"${deployOutput.deploymentRollupManagerBlockNumber}"`
  );

  fs.writeFileSync(configTomlPath, configContent, "utf8");
}

function updateBridgeConfigToml(l1RpcUrl: string, configTomlPath: string, config: IConfig) {
  const { createRollupOutput, deployOutput, genesisOutput } = config;
  let configContent = fs.readFileSync(configTomlPath, "utf8");

  configContent = configContent.replace(
    /(GenBlockNumber\s*=\s*)\d+/,
    `$1${deployOutput.deploymentRollupManagerBlockNumber}`
  );
  configContent = configContent.replace(
    /(PolygonBridgeAddress\s*=\s*)".*"/,
    `$1"${deployOutput.polygonZkEVMBridgeAddress}"`
  );
  configContent = configContent.replace(
    /(PolygonZkEVMGlobalExitRootAddress\s*=\s*)".*"/,
    `$1"${deployOutput.polygonZkEVMGlobalExitRootAddress}"`
  );
  configContent = configContent.replace(
    /(PolygonRollupManagerAddress\s*=\s*)".*"/,
    `$1"${deployOutput.polygonRollupManagerAddress}"`
  );
  configContent = configContent.replace(
    /(PolygonZkEvmAddress\s*=\s*)".*"/,
    `$1"${createRollupOutput.rollupAddress}"`
  );
  ...
  if (bridgeProxy) {
    ...
    configContent = configContent.replace(
      /(L2PolygonBridgeAddresses\s*=\s*)\[".*"\]/,
      `$1["${bridgeProxy.address}"]`
    );
  }
...
task("updateConfigTask", "update config for network")
  ...
  .addOptionalParam<string>("wsrpc", "L1 WS RPC URL")
