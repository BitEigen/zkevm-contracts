...
async function createDeployParamTask(taskArgs: {chainid: string}, hre: HardhatRuntimeEnvironment) {
    ...
    fs.writeFileSync(
        path.join(__dirname, "../deployment/v2/deploy_parameters.json"),
        JSON.stringify(deployParams, null, 1)
    );
    fs.writeFileSync(
        path.join(__dirname, "../deployment/v2/create_rollup_parameters.json"),
        JSON.stringify(createRollupParams, null, 1)
    );
}
